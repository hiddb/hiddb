...
pub struct ReverseSortedList<K: PartialOrd, V> {
    data: Vec<(K, V)>,
}

impl<K: PartialOrd, V> ReverseSortedList<K, V>
where
    K: Copy,
    // V: Copy,
{
    ...
    pub fn insert(&mut self, key_value_pair: (K, V)) {
        let idx = match self.data.binary_search_by(|entry| {
            key_value_pair.0.partial_cmp(&entry.0).unwrap() // unwrap for NaN
        }) {
...
impl<K: PartialOrd, V, Idx> std::ops::Index<Idx> for ReverseSortedList<K, V>
where
...
impl<K: PartialOrd, V> FromIterator<(K, V)> for ReverseSortedList<K, V>
where
    K: Copy,
    V: Copy,
{
    fn from_iter<I: IntoIterator<Item = (K, V)>>(iter: I) -> Self {
        let mut c = ReverseSortedList::new();
...
mod tests {
    ...
    #[test]
    fn test_all() {
        let mut sorted_list = ReverseSortedList::<f64, u64>::new();
        // Reverse order is: 2 0 1
        sorted_list.insert((0.5, 0));
        assert_eq!(sorted_list[0].1, 0);

        sorted_list.insert((0.1, 1));
        assert_eq!(sorted_list[0].1, 0);
        assert_eq!(sorted_list[1].1, 1);

        sorted_list.insert((1.2, 2));
        assert_eq!(sorted_list[0].1, 2);
        assert_eq!(sorted_list[1].1, 0);
        assert_eq!(sorted_list[2].1, 1);
        assert_eq!(sorted_list[1..][0].1, 0);
        assert_eq!(sorted_list[1..][1].1, 1);
        assert_eq!(sorted_list.n_first(2)[0].1, 2);
        assert_eq!(sorted_list.n_first(2)[1].1, 0);

        assert_eq!(sorted_list.first().1, 2);
        assert_eq!(sorted_list.last().1, 1);
